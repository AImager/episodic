...
- **一键收藏（v2.2）**：点「⭐ 收藏当前页面」把任意网页存成一条指针（日期/标题/来源/链接），追加到仓库根目录 `收藏/README.md` 的表格里。按 URL 去重，已收藏过不重复。**这是纯指针归档，落点和对话存档隔离，不参与知识提炼**——就是个同步到 GitHub 的电子收藏夹。
...
2. 点扩展图标 → 「⭐ 收藏当前页面（存指针）」
3. 一条记录（日期/标题/来源/链接）追加到仓库根 `收藏/README.md`，`git pull` 即进 vault

> 这是**纯指针归档**——只存链接不存正文，落在独立的 `收藏/` 目录，和对话存档（要提炼的真原料）隔离。定位就是个同步到 GitHub 的电子收藏夹，不参与知识提炼。设计动机见 [CHANGELOG.md](./CHANGELOG.md)。
...
```
manifest.json    扩展配置
background.js     核心：导出逻辑 + GitHub 同步 + 收藏
popup.html/js     弹窗：去向选择 + 导出按钮 + 收藏按钮
options.html/js   设置页：GitHub 配置 + 测试连接
...
完整版本历史与每版的需求动机见 [CHANGELOG.md](./CHANGELOG.md)。当前版本 **v2.2.0**（新增一键收藏）。
